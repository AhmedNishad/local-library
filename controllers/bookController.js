...
exports.book_detail = function(req, res, next) {
    ...
    async.parallel({
        ...
        // Successfully retrieved book object
...
exports.book_delete_get = function(req, res, next) {
    async.parallel({
        ...
        bookInstances: function(callback){
            Book.find({'bookinstances': req.params.id}).exec(callback)
        },
    },function(err, result){
        if(err) {next(err)}
        ...
        res.render('book_delete', {title: 'Delete Book', book: result.book, book_instances: result.bookInstances})
    }
    )
    ...
    (req, res, next) => {
        ...
        if (!errors.isEmpty()) {
            ...
            async.parallel({
                ...
                // Mark our selected genres as checked.
                for (let i = 0; i < results.genres.length; i++) {
                    // if genre is in array of genres
                    if (book.genre.indexOf(results.genres[i]._id) > -1) {
                        results.genres[i].checked='true';
                    }
                }
